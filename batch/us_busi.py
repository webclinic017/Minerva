...
def make_plot(ticker, df, proj, tech_type, sbubplot_cnt, idx):
    ...
    elif proj == 'STAY':  # 상하위 값이 없는 경우, 즉 threshold 를 crossover 만 탐색하는 경우 사용하는 루틴: trend 불필요 
        h_limit = 0
        l_limit = 0
        try:
    ...
    plt.title(f"{ticker}: {tech_type} / {proj} / {txt}", fontdict={'fontsize':20, 'color':'g'})    
    ...
    for x in WATCH_TICKERS['US']:
        ...
        for key, item in x.items():
            ...
            for ticker in item:
                ...
                logger2.info(f'##### {nation} Economic Trend: {proj}')        
                ...
                # Volume.Volume Weighted Average Price (VWAP)
                tech_type = 'vwap'
                df, latest_date = vwap(ticker, df, proj, tech_type, sbubplot_cnt, 6)      
                print(latest_date)
